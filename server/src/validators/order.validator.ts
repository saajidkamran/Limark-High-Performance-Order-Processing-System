...
// Constants
const VALID_STATUSES: readonly Order['status'][] = ['PENDING', 'PROCESSING', 'COMPLETED', 'FAILED'] as const;
const MIN_BATCH_SIZE = 1;
const MAX_BATCH_SIZE = 1000;

/**
 * Pure function: Validates an order structure
 */
export const validateOrder = (order: Order): boolean => {
  ...
  const hasValidCreatedAt = typeof order.createdAt === 'number' && order.createdAt > 0;
  const hasValidUpdatedAt = typeof order.updatedAt === 'number' && order.updatedAt > 0;

  return hasValidId && hasValidStatus && hasValidAmount && hasValidCreatedAt && hasValidUpdatedAt;
};

/**
 * Pure function: Validates batch size
 */
export const validateBatchSize = (batchSize: number | undefined): BatchSizeValidation => {
